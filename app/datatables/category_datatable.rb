class CategoryDatatable < AjaxDatatablesRails::Base
  ...
  def_delegators :@view, :truncate, :link_to, :h, :boolean_ajax_link_to, :link_with_icon, :category_path, :edit_category_path, :delete_category_path
    ...
    records.map do |record|
      [
        ...
        # example: record.attribute,
          ...
          boolean_ajax_link_to(record.is_public, '#'),
          boolean_ajax_link_to(record.is_locked, '#'),
          boolean_ajax_link_to(record.is_trend, '#'),
          boolean_ajax_link_to(record.is_active, '#'),
  ...
  def get_raw_records
    Category.where(parent_id: options[:parent_id])
        .joins("LEFT JOIN categories parent ON categories.parent_id = parent.id")
